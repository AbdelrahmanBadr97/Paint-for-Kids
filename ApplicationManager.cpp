...
void ApplicationManager::CheckRotate()
{
	Point p;
	do {
		pIn->GetPointClicked(p.x, p.y);
		if (p.y > pOut->getUIToolBarHeight())//Click on inplay toolbar
		{
			continue;
		}
		if (Rotate_BACK == (p.x / pOut->getUIMenuItemWidth()))
		{
		...
		int successCount = 0;
		string msg;
		if (Rotate_90 == (p.x / pOut->getUIMenuItemWidth()))
		{
			angle = 90;
			msg = "90 degree rotation";
		}
		if (Rotate_180 == (p.x / pOut->getUIMenuItemWidth()))
		{
			angle = 180;
			msg = "180 degree rotation";
		}
		if (Rotate_270 == (p.x / pOut->getUIMenuItemWidth()))
		{
			angle = 270;
		...
		for (int i = 0; i < SelectedCount; i++)
		{
			if (SelectedFigs[i]->Rotate(angle))
			{
				successCount++;
			}
		}
		int failedCount = SelectedCount - successCount;
		UpdateInterface();
